...
const fetchData = async (params = null) => {
  return await fetch(`${process.env.BASE_URL}/rivojlanishi-list`).then((res) =>
    res.json()
  );
};
...
export const generateMetadata = async ({ params }) => {
  ...
  const article = data?.find(
    (article) => article?.title == params?.rivojlanishi.replace(/-/g, " ")
  );
...
const Rivojlanishi = async ({ params = null }) => {
  const data = await fetchData(params);
