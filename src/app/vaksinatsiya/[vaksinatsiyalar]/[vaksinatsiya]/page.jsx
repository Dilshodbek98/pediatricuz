...
const fetchData = async (params) => {
  return await fetch(`${process.env.BASE_URL}/vaksinatsiya-list`).then((res) =>
    res.json()
  );
};
...
export const generateMetadata = async ({ params = null }) => {
  ...
  const article = data?.find(
    (article) => article?.title == params?.vaksinatsiya.replace(/-/g, " ")
  );
...
const Vaksinatsiya = async ({ params = null }) => {
  const data = await fetchData(params);
