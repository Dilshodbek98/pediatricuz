...
const fetchData = async (params) => {
  return await fetch(`${process.env.BASE_URL}/kasalliklar-list`).then((res) =>
    res.json()
  );
};
...
export const generateMetadata = async ({ params = null }) => {
  ...
  const article = data?.find(
    (article) => article?.title == params?.kasallik.replace(/-/g, " ")
  );
...
const Kasallik = async ({ params = null }) => {
  const data = await fetchData(params);
